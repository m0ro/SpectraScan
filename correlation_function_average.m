function vector=correlation_function_average(matrix3D,L,time)
n=size(matrix3D);
C=zeros(n(3)*(n(3)+1)/2,2);
i=0;
for k=1:n(3) % loop on the differents omega of the 3Dmatrix
    for q=1:n(3)-k % loop on the different steps between different omegas
        i=i+1;
